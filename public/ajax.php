...
try {
    match ($action) {
        ...
        'login' => handle_login($users, $rateLimiter),
    ...
    app_log('ajax.php action failed: ' . $action, $exception);
...
/**
 * EN: Authenticates an existing account with brute-force throttling and blocks suspended users.
 * RU: Авторизует существующий аккаунт с защитой от перебора и запрещает вход заблокированным пользователям.
 *
 * @param UserRepository $users User repository / Репозиторий пользователей
 * @param RateLimiter $rateLimiter Login throttle / Ограничитель попыток входа
 * @return never
 */
function handle_login(UserRepository $users, RateLimiter $rateLimiter): never
{
    $email = mb_strtolower(Validator::text($_POST['email'] ?? '', 180), 'UTF-8');
    $password = (string) ($_POST['password'] ?? '');
    $ip = (string) ($_SERVER['REMOTE_ADDR'] ?? '');

    try {
        if ($rateLimiter->isLoginBlocked($email, $ip)) {
            Response::json(['ok' => false, 'message' => 'Тым көп сәтсіз әрекет. 15 минуттан кейін қайталаңыз.'], 429);
        }
    } catch (\PDOException $exception) {
        app_log('Login rate limiter unavailable, failing open.', $exception);
    }
    ...
    if (!$user || !password_verify($password, (string) $user['password_hash'])) {
        try {
            $rateLimiter->recordLogin($email, $ip, false);
        } catch (\PDOException $exception) {
            app_log('Login rate limiter write failed.', $exception);
        }
    ...
    try {
        $rateLimiter->recordLogin($email, $ip, true);
    } catch (\PDOException $exception) {
        app_log('Login rate limiter write failed.', $exception);
    }
...
function handle_owner_user_reset_password(OwnerRepository $owner, NotificationRepository $notifications): never
{
    ...
    $notifications->notify(
        $userId,
        'password_reset',
        'Пароль сброс жасалды',
        'Owner аккаунтыңызға уақытша пароль орнатты. Жаңа пароль owner арқылы жеке хабарланады, кіргеннен кейін оны бірден ауыстырыңыз.',
        true
    );
...
/**
 * EN: Requires any authenticated non-blocked account and re-validates status against the database,
 *     so blocking takes effect immediately even for already-open sessions.
 * RU: Требует любой авторизованный незаблокированный аккаунт и перепроверяет статус в базе,
 *     чтобы блокировка действовала сразу даже для уже открытых сессий.
 *
...
function require_active_account(): array
{
    ...
    if (!$fresh) {
        Auth::logout();
        Response::json(['ok' => false, 'message' => 'Аккаунт табылмады. Қайта кіріңіз.'], 403);
    }

    if (($fresh['status'] ?? 'active') === 'blocked') {
        Auth::logout();
        Response::json(['ok' => false, 'message' => 'Аккаунт бұғатталған.'], 403);
    }

    $_SESSION['user']['status'] = (string) $fresh['status'];
    $_SESSION['user']['role'] = (string) $fresh['role'];

    return array_merge($user, ['status' => (string) $fresh['status'], 'role' => (string) $fresh['role']]);
}
